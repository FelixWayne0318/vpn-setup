...
| **OpenAI Codex** (VS Code 扩展) | 登录超时 `Operation timed out` |
...
`auth.openai.com` 被 Cloudflare Challenge 保护（[已知 Bug](https://github.com/openai/codex/issues/16052)），OAuth 登录会超时。

```bash
# 推荐方案: 用手机热点临时登录（使用 ChatGPT 订阅额度，不额外花钱）
mkdir -p ~/.codex
cat > ~/.codex/config.toml << 'EOF'
forced_login_method = "chatgpt"
cli_auth_credentials_store = "file"
EOF
# 然后: 手机开热点 → 暂停 Clash → Codex 登录 → 重开 Clash
# 凭证缓存在 ~/.codex/auth.json，后续自动复用

# 备选方案: API Key（独立计费，与 ChatGPT 订阅分开）
