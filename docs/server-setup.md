...
脚本会自动安装:
- **3x-ui 面板** (内置 Xray，Web 界面管理节点，仅本地监听)
- **Cloudflare WARP** (AI 域名走干净 IP)
- **UFW 防火墙** 规则 (3000/8000 端口 DENY，x-ui 面板不暴露公网)

### 5. 配置 x-ui 面板

x-ui 面板仅监听 127.0.0.1，需通过 SSH 隧道访问:

```bash
# 建立 SSH 隧道
ssh -L 19921:127.0.0.1:19921 linuxuser@你的服务器IP -p 22022

# 然后浏览器打开
http://127.0.0.1:19921/你的随机路径/
```
